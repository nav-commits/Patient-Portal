...
import { Box, Heading, Text, VStack, Table, HStack, Spinner, Center } from "@chakra-ui/react";
...
export default function TestPage() {
  ...
  const testName = params.testName ? decodeURIComponent(params.testName as string) : undefined;
          ...
          <Box mt={8} borderWidth={1} borderRadius="lg" p={4}>
            ...
            <Chart.Root mt={12} maxH="md" chart={chart}>
              <LineChart data={chart.data}>
                ...
                <XAxis dataKey={chart.key("date")} axisLine={false} tickLine={false} stroke={chart.color("border")} />
                ...
                {chart.series.map((item) => (
                  <Line key={item.name} isAnimationActive={false} dataKey={chart.key(item.name)} stroke={chart.color(item.color)} strokeWidth={2} dot={{ r: 4 }} />
                ))}
              </LineChart>
            </Chart.Root>
          </Box>
